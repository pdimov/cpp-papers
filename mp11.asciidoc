...
```
[horizontal]
_Returns:_:: `mp_true` when `F<T...>` is a valid expression, `mp_false` when `F<T...>` causes a substitution failure.
...
[horizontal]
_Returns:_::
* when `F<T...>` is a valid expression,
+
[subs=+quotes]
```
...
```
[horizontal]
_Returns:_:: `mp_true` when `L` is a _list_ (an instantiation of a class template whose parameters are all types), `mp_false` otherwise.
...
_Remarks:_:: `mp_nth_element_c` is not required to evaluate the exact expression `mp_at_c<mp_sort<L, P>, I>`.
...
[ _Note:_ `mp_less<T1, T2>` is not necessarily the same as `mp_less<(T1::value < T2::value)>` when comparing between signed and unsigned types;
